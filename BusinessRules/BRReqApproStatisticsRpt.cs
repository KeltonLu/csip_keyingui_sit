...
namespace CSIPKeyInGUI.BusinessRules
{
    ...
    public class BRReqApproStatisticsRpt : CSIPCommonModel.BusinessRules.BRBase<EntityCUSTOMER_LOG>
    {
        ...
        private const string SEL_USER_STATISTICS = @"SELECT A.user_id user_id,B.USER_NAME " +
                ...
                "UNION " +
                "SELECT I.user_id user_id,J.USER_NAME " +
                "FROM shop_reqAppro I,(select distinct user_id,User_name from {0}.dbo.M_USER) as  J " +
                "where I.mod_date between @SearchStart and @SearchEnd " +
                    "AND I.user_id = J.USER_ID " +
                "UNION " +
                                ...
                                string strSearchEnd, ref string strMsgID)
        {
            try
            {
                ...
                Logging.Log("========== 執行經辦作業量統計(特店資料)-列印 ==========\r查詢經辦訊息" + recordSQL);
                ...
                for (int intLoop = 0; intLoop < dstSearchResult.Tables[0].Rows.Count; intLoop++)
                {
                    ...
                    if (intLoop == 0)
                    {
                        ...
                        Logging.Log("========== 執行經辦作業量統計(特店資料)-列印 ==========\r查詢BlockCode解除管制(卡片資料異動)、特店資料異動(資料異動)、費率一Key、二Key（特店資料異動）\r" + loopRecordSQL);
                    }
                    #endregion 記錄SQL到Default Log

                    //* 執行查詢SQL語句
                    DataSet dstResult = BRReqApproStatisticsRpt.SearchOnDataSet(sqlcmdSearch);
                    if (null == dstResult)
                    {
                        strMsgID = "01_03030000_002";
                        return null;
                    ...
                    if (intLoop == 0)
                    {
                        loopRecordSQL = sqlcmdSearch.CommandText;
                        loopRecordSQL = loopRecordSQL.Replace("@SearchStart", string.Format("'{0}'", strSearchStart.Replace("/", ""))).Replace("@SearchEnd", string.Format("'{0}'", strSearchEnd.Replace("/", "")));
                        Logging.Log("========== 執行經辦作業量統計(特店資料)-列印 ==========\r查詢特店資料異動(請款加批異動)、特店資料新增(請款加批)\r" + loopRecordSQL);
                    }
                    #endregion 記錄SQL到Default Log
                    ...
                    if (intLoop == 0)
                    {
                        loopRecordSQL = sqlcmdSearch.CommandText;
                        loopRecordSQL = loopRecordSQL.Replace("@SearchStart", string.Format("'{0}'", strSearchStart.Replace("/", ""))).Replace("@SearchEnd", string.Format("'{0}'", strSearchEnd.Replace("/", "")));
                        Logging.Log("========== 執行經辦作業量統計(特店資料)-列印 ==========\r查詢帳號一Key、二Key（特店資料異動）、解約一Key、二Key（特店資料異動）、機器一Key、二Key（特店資料異動）\r" + loopRecordSQL);
                    }
                    #endregion 記錄SQL到Default Log
                    ...
                    if (intLoop == 0)
                    {
                        loopRecordSQL = sqlcmdSearch.CommandText;
                        loopRecordSQL = loopRecordSQL.Replace("@SearchStart", string.Format("'{0}'", strSearchStart.Replace("/", ""))).Replace("@SearchEnd", string.Format("'{0}'", strSearchEnd.Replace("/", "")));
                        Logging.Log("========== 執行經辦作業量統計(特店資料)-列印 ==========\r查詢特店資料新增(6001)、特店資料新增(6001會員附加服務)、特店資料新增(PCAM)\r" + loopRecordSQL);
                    }
                    #endregion 記錄SQL到Default Log
                ...
                stopWatch.Stop();
                Logging.Log(string.Format("經辦作業量統計(特店資料)-列印的資料查詢、組成，耗時： {0} 毫秒", stopWatch.ElapsedMilliseconds)); //20210531_Ares_Stanley-增加經辦作業量統計(特店資料)查詢/列印的資料查詢、組成耗時記錄
                return dtblResult;
        ...
        /// 取報表數據-僅供查詢使用
        /// 修改紀錄:2021/02/01_Ares_Stanley-移除移除請款加批、延伸性作業/請款加批加總、6001會員附加服務
        /// </summary>
        /// <param name="strSearchStart">區間起</param>
        /// <param name="strSearchEnd">區間迄</param>
        /// <param name="strMsgID">返回的錯誤ID號</param>
        /// <returns>成功時：返回查詢結果；失敗時：null</returns>
        public static DataTable SearchRPTData_Search(string strSearchStart,
                                string strSearchEnd, ref string strMsgID, int iPageIndex, int iPageSize, ref int iTotalCount)
        {
            try
            {
                ...
                string recordSQL = sqlcmdSearch.CommandText;
                recordSQL = recordSQL.Replace("@SearchStart", string.Format("'{0}'", strSearchStart.Replace("/", ""))).Replace("@SearchEnd", string.Format("'{0}'", strSearchEnd.Replace("/", ""))).Replace("@user_id", "");
                Logging.Log("========== 執行經辦作業量統計(特店資料)-查詢 ==========\r查詢經辦訊息" + recordSQL);
                #endregion 記錄SQL到Default Log
                Stopwatch stopWatch = new Stopwatch();
                stopWatch.Start();
                //* 執行查詢SQL語句
                ...
                for (int intLoop = 0; intLoop < dstSearchResult.Tables[0].Rows.Count; intLoop++)
                {
                    ...
                    if(intLoop == 0)
                    {
                        loopRecordSQL = sqlcmdSearch.CommandText;
                        loopRecordSQL = loopRecordSQL.Replace("@SearchStart", string.Format("'{0}'", strSearchStart.Replace("/", ""))).Replace("@SearchEnd", string.Format("'{0}'", strSearchEnd.Replace("/", "")));
                        Logging.Log("========== 執行經辦作業量統計(特店資料)-查詢 ==========\r查詢BlockCode解除管制(卡片資料異動)、特店資料異動(資料異動)、費率一Key、二Key（特店資料異動）\r" + loopRecordSQL);
                    }
                    #endregion 記錄SQL到Default Log
                    ...
                    if (intLoop == 0)
                    {
                        loopRecordSQL = sqlcmdSearch.CommandText;
                        loopRecordSQL = loopRecordSQL.Replace("@SearchStart", string.Format("'{0}'", strSearchStart.Replace("/", ""))).Replace("@SearchEnd", string.Format("'{0}'", strSearchEnd.Replace("/", "")));
                        Logging.Log("========== 執行經辦作業量統計(特店資料)-查詢 ==========\r查詢特店資料異動(請款加批異動)、特店資料新增(請款加批)\r" + loopRecordSQL);
                    }                    
                    #endregion 記錄SQL到Default Log
                    ...
                    if (intLoop == 0)
                    {
                        loopRecordSQL = sqlcmdSearch.CommandText;
                        loopRecordSQL = loopRecordSQL.Replace("@SearchStart", string.Format("'{0}'", strSearchStart.Replace("/", ""))).Replace("@SearchEnd", string.Format("'{0}'", strSearchEnd.Replace("/", "")));
                        Logging.Log("========== 執行經辦作業量統計(特店資料)-查詢 ==========\r查詢帳號一Key、二Key（特店資料異動）、解約一Key、二Key（特店資料異動）、機器一Key、二Key（特店資料異動）\r" + loopRecordSQL);
                    }                    
                    #endregion 記錄SQL到Default Log
                    ...
                    if (intLoop == 0)
                    {
                        loopRecordSQL = sqlcmdSearch.CommandText;
                        loopRecordSQL = loopRecordSQL.Replace("@SearchStart", string.Format("'{0}'", strSearchStart.Replace("/", ""))).Replace("@SearchEnd", string.Format("'{0}'", strSearchEnd.Replace("/", "")));
                        Logging.Log("========== 執行經辦作業量統計(特店資料)-查詢 ==========\r查詢特店資料新增(6001)、特店資料新增(6001會員附加服務)、特店資料新增(PCAM)\r" + loopRecordSQL);
                    }                    
                    #endregion 記錄SQL到Default Log
                ...
                dtblResult.Columns.Add("rowSum_1key", typeof(string));
                dtblResult.Columns.Add("rowSum_2key", typeof(string));
                for (int i = 0; i < dtblResult.Rows.Count; i++)
                {
                    dtblResult.Rows[i]["rowSum_1key"] = (int.Parse(dtblResult.Rows[i]["D01"].ToString()) + int.Parse(dtblResult.Rows[i]["D03"].ToString()) + int.Parse(dtblResult.Rows[i]["D05"].ToString()) + int.Parse(dtblResult.Rows[i]["D07"].ToString()) + int.Parse(dtblResult.Rows[i]["D09"].ToString()) + int.Parse(dtblResult.Rows[i]["D11"].ToString()) + int.Parse(dtblResult.Rows[i]["D13"].ToString())).ToString("N0");
                    dtblResult.Rows[i]["rowSum_2key"] = (int.Parse(dtblResult.Rows[i]["D02"].ToString()) + int.Parse(dtblResult.Rows[i]["D04"].ToString()) + int.Parse(dtblResult.Rows[i]["D06"].ToString()) + int.Parse(dtblResult.Rows[i]["D08"].ToString()) + int.Parse(dtblResult.Rows[i]["D10"].ToString()) + int.Parse(dtblResult.Rows[i]["D12"].ToString()) + int.Parse(dtblResult.Rows[i]["D14"].ToString())).ToString("N0");
                }
                // 頁數計算
                ...
                stopWatch.Stop();
                Logging.Log(string.Format("經辦作業量統計(特店資料)-查詢的資料查詢、組成，耗時： {0} 毫秒", stopWatch.ElapsedMilliseconds)); //20210531_Ares_Stanley-增加經辦作業量統計(特店資料)查詢/列印的資料查詢、組成耗時記錄
                return dtblResult;
