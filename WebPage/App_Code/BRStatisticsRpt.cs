...
//*  功能說明：作業量統計表
//*  創建日期：2018/05/02
//*  修改記錄：此檔案為BusinessRules.BRStatisticsRpt的新版
...
namespace CSIPKeyInGUI.BusinessRules_new
{
    ...
    public class BRStatisticsRpt : CSIPCommonModel.BusinessRules.BRBase<EntityCUSTOMER_LOG>
    {
        ...
        // 1641: eTag儲值001001
        private const string SEL_UTILITIES_1641 = @"select '1' flag, count(receive_number) from utilities 
                                                    where keyin_date between @SearchStart and @SearchEnd 
                                                    and keyIn_flag = '1' and sendhost_Flag = 'Y' 
                                                    and business_category = '001001' 
                                                    union 
                                                    select '2' flag, count(receive_number) from utilities 
                                                    where keyin_date between @SearchStart and @SearchEnd 
                                                    and keyIn_flag = '1' and sendhost_Flag = 'Y' 
                                                    and business_category = '001001' 
                                                    order by flag ";
        ...
        // 1642: 臨時停車001002
        private const string SEL_UTILITIES_1642 = @"select '1' flag, count(receive_number) from utilities 
                                                    where keyin_date between @SearchStart and @SearchEnd 
                                                    and keyIn_flag = '1' and sendhost_Flag = 'Y' 
                                                    and business_category = '001002' 
                                                    union 
                                                    select '2' flag, count(receive_number) from utilities 
                                                    where keyin_date between @SearchStart and @SearchEnd 
                                                    and keyIn_flag = '1' and sendhost_Flag = 'Y' 
        ...
        // 1643: 月租停車001003
        private const string SEL_UTILITIES_1643 = @"select '1' flag, count(receive_number) from utilities 
                                                    where keyin_date between @SearchStart and @SearchEnd 
                                                    and keyIn_flag = '1' and sendhost_Flag = 'Y' 
                                                    and business_category = '001003' 
                                                    union 
                                                    select '2' flag, count(receive_number) from utilities 
                                                    where keyin_date between @SearchStart and @SearchEnd 
                                                    and keyIn_flag = '1' and sendhost_Flag = 'Y' 
                                ...
                                string strSearchEnd, ref string strMsgID)
        {
            try
            {
                ...
                sbSqlCommand = new StringBuilder();
                sbSqlCommand.Append(SEL_UTILITIES_1641);// eTag儲值
                sbSqlCommand.Append(SEL_UTILITIES_1642);// 臨時停車
                sbSqlCommand.Append(SEL_UTILITIES_1643);// 月租停車
                sqlcmdSearch.CommandText = sbSqlCommand.ToString();
                sqlcmdSearch.CommandType = CommandType.Text;
                ...
                intCount = 0;
                intCountI = 0;
                DataTable dtSearchResult = dstSearchResult.Tables[0];
                for (int i = 0; i < dtSearchResult.Rows.Count; i++)
                {
                    if (dtSearchResult.Rows[i][0].ToString().Trim() == "1")
                        intCount += Convert.ToInt32(dtSearchResult.Rows[i][1]);
                    else
                        intCountI += Convert.ToInt32(dtSearchResult.Rows[i][1]);
                }

                drowInsert = dtblResult.NewRow();
                drowInsert["trans_sum"] = intCountI;
                drowInsert["trans_name2"] = "公用事業(ETAG業務)";
                drowInsert["trans_1keysum"] = intCount;
                drowInsert["trans_name"] = "eTag儲值";
                drowInsert["rpt_order"] = 28;
                dtblResult.Rows.Add(drowInsert);
                ...
                intCount = 0;
                intCountI = 0;
                dtSearchResult = dstSearchResult.Tables[1];
                for (int i = 0; i < dtSearchResult.Rows.Count; i++)
                {
                    if (dtSearchResult.Rows[i][0].ToString().Trim() == "1")
                        intCount += Convert.ToInt32(dtSearchResult.Rows[i][1]);
                ...
                drowInsert["trans_name2"] = "公用事業(ETAG業務)";
                drowInsert["trans_1keysum"] = intCount;
                drowInsert["trans_name"] = "臨時停車";
                drowInsert["rpt_order"] = 29;
                dtblResult.Rows.Add(drowInsert);

                // 月租停車
                intCount = 0;
                intCountI = 0;
                dtSearchResult = dstSearchResult.Tables[2];
                for (int i = 0; i < dtSearchResult.Rows.Count; i++)
                {
                    if (dtSearchResult.Rows[i][0].ToString().Trim() == "1")
                        intCount += Convert.ToInt32(dtSearchResult.Rows[i][1]);
                ...
                drowInsert["trans_name2"] = "公用事業(ETAG業務)";
                drowInsert["trans_1keysum"] = intCount;
                drowInsert["trans_name"] = "月租停車";
                drowInsert["rpt_order"] = 30;
                dtblResult.Rows.Add(drowInsert);
