//2021/04/01_Ares_Stanley-移除MicrosoftExcel
using System;
...
/// <summary>
/// BRArtificial_Signing_Batch_Data 的摘要描述
/// </summary>
...
{ 
    ...
    public const string SEL_ASBD_Import_CheckCross = @"Select Shop_ID from Artificial_Signing_Batch_Data Where Batch_Date = @Batch_Date   and Batch_NO = @Batch_NO and Sign_Type = @Sign_Type ";
    ...
    //        Logging.Log(" xlApp 讀檔案完成， " + strPath);
    ...
    //        else
    //        {
    //            Logging.Log("匯入異常 xlApp讀取異常 ");
    //        }
    //    }
    //}

    #region 透過NPOI從EXCEL檔匯入資料
    /// <summary>
    /// 建立日期:2021/03/30_Ares_Stanley
    /// 由EXCEL檔匯入資料
    /// </summary>
    /// <param name="strPath">EXCEL檔路徑</param>
    /// <param name="FileName">EXCEL檔名</param>
    ...
    /// <param name="updatefail">更新失敗筆數</param>
    /// <returns>true:匯入成功,false:匯入失敗</returns>
    public static bool Import_NPOI(string strPath, string userid, ref string strMsgID, ref int total, ref int insertcount, ref int updatecount, ref int insertfail, ref int updatefail)
    {
        //解決"格式太舊或是類型程式庫無效。 (Exception from HRESULT: 0x80028018 (TYPE_E_INVDATAREAD))"問題用
        System.Globalization.CultureInfo oldCI = System.Threading.Thread.CurrentThread.CurrentCulture;
        System.Threading.Thread.CurrentThread.CurrentCulture = new System.Globalization.CultureInfo("en-US");
    ...
    /// <param name="updatefail">更新失敗筆數</param>
    /// <returns>true:匯入成功,false:匯入失敗</returns>
    /// 修改日期: 2021/03/30_Ares_Stanley-變更Excel資料匯入方式為NPOI
    public static bool ImportCross(string strPath, string userid, ref string strMsgID, ref int total, ref int insertcount, ref int updatecount, ref int insertfail, ref int updatefail, ref ToACQRtn JsonObj)
    {
        #region 舊CODE
        ////解決"格式太舊或是類型程式庫無效。 (Exception from HRESULT: 0x80028018 (TYPE_E_INVDATAREAD))"問題用
        //System.Globalization.CultureInfo oldCI = System.Threading.Thread.CurrentThread.CurrentCulture;
        ...
        //                //若無資料則新增,若有資料則只更新該筆資料的商店代號
        //                DataSet ds = Select_Import_CheckCross(Batch_Date, "", Batch_NO, Sign_Type);
        //                if (ds.Tables[0].Rows.Count > 0)
        //                {
        //                    if (ds.Tables[0].Rows[0][0].ToString() != Shop_ID)
        //                    {
        //                        //有資料且商店代號不同只更新該筆資料的商店代號，產生UPDATE語法
        //                        //2021/03/09_Ares_Stanley-DB名稱改為變數
        //                        sqlcmm.CommandText = string.Format("Update [{7}].[dbo].[Artificial_Signing_Batch_Data] Set [Shop_ID] = '{0}',[Modify_User] = '{1}',[Modify_DateTime] = '{2}' Where Batch_Date = '{3}'  {4} and Batch_NO = {5} and Sign_Type = {6};"
        //                            , Shop_ID, userid, importDatetime, Batch_Date, "", Batch_NO, Sign_Type, UtilHelper.GetAppSettings("DB_KeyinGUI"));
        //                        if (Update(sqlcmm))
        //                        {
        //                            updatecount++;
        //                        }
        ...
        //                        //批數+1
        //                        JsonObj.TotalBatCount++;
        //                        //加請款筆數
        //                        int tmpTC = 0;
        //                        int.TryParse(Receive_Total_Count, out tmpTC);
        //                        JsonObj.TotalapplyCount += tmpTC;
        //                        //加請款金額
        //                        decimal tmpTM = 0;
        //                        decimal.TryParse(Receive_Total_AMT, out tmpTM);
        //                        JsonObj.TotalapplyAMT += tmpTM;
        //                        //加入物件 
        //                        JsonObj.ADD(Mta);
        //                    }
        //                    else
    ...
    /// <param name="strBatch_NO">批號</param>
    /// <returns></returns>        
    public static DataSet Select_Import_CheckCross(string strBatch_Date, string strReceive_Batch, string strBatch_NO, string strSign_Type)
    {
        DataSet ds = null;
        SqlHelper sSql = new SqlHelper();
        ...
        try
        {
            sqlComm.CommandText = SEL_ASBD_Import_CheckCross;
            sqlComm.CommandType = CommandType.Text;
