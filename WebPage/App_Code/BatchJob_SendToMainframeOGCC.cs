...
/// 傳送給主機，法律形式及AML行業別，代號及中文對照表
/// 檔名：AML_DESC.TXT
/// </summary>
public class BatchJob_SendToMainframeOGCC : Quartz.IJob
{
    ...
    protected string _MailTitle = "傳送法律形式及AML行業別設定檔 批次：";
    ...
    public void Execute(JobExecutionContext context)
    {
        ...
        try
        {
            JobHelper.Write(JobID, "*********** " + JobID + " 傳送法律形式及AML行業別設定檔 批次 START ************** ", LogState.Info);
            ...
            if (isContinue)
            {
                // 取得法律形式及AML行業別設定檔資料( to 主機)
                postData = PostMainframeOGCCService.SendPostOfficeTypeSetting();

                //根據資料組成檔案，並上傳至FTP上
                PostFileName = "AML_DESC.txt";//20200327 主機要求，副檔名要小寫
                isComplete = GenaratorFileAndUploadToMFTP("PostMainframeOGCC", PostFileName, postData, _PostMainframeOGCCService);
                ...
                {
                    InsertBatchLog(JobID, total, total, "F", "無法律形式及AML行業別設定檔資料");

                    JobHelper.Write(JobID, _MailTitle + " 失敗，今日無主機法律形式及AML行業別設定檔資料！", LogState.Error);
                    _PostMainframeOGCCService.SendMail(_MailTitle + " 失敗，無法律形式及AML行業別設定檔資料！", _MailTitle + PostFileName + " 失敗，請確認CSIP的法律形式及AML行業別設定檔資料", "上傳失敗", this.StartTime);
                }
            }
        }
        catch (Exception ex)
        {
            ...
            JobHelper.Write(JobID, "【FAIL】傳送法律形式及AML行業別設定檔 批次" + fileName + " 發生錯誤：" + ex.Message);
            // 發送 Email
        ...
        {
            JobHelper.Write(JobID, "傳送法律形式及AML行業別設定檔 批次 Job 結束！", LogState.Info);
    ...
    private bool CheckJobIsContinue(string JobID, ref string msgID)
    {
        ...
        {
            result = false;
            JobHelper.Write(JobID, "【FAIL】" + ex.ToString(), LogState.Error);
        }
    ...
    private bool GenaratorFileAndUploadToMFTP(string jobID, string fileName, DataTable postData, PostMainframeOGCCService _PostMainframeOGCCService)
    {
        ...
        else
        {
            ...
            foreach (DataRow row in postData.Rows)
            {
                ...
                postDataContent += CHT_WordPadLeftRight(ToWide(row["CODE_NAME"].ToString().Trim()), "L", 86, ' ');   // 3.代碼中文
        ...
        {
            JobHelper.Write(jobID, "[FAIL] FileName：" + fileName + "，上送FTP 失敗", LogState.Error);
            isComplete = false;
        }
