...
//* Ares_Luke          2021/04/14           20200031-CSIP EOS       新增 JobHelper.strJobID
//*******************************************************************
...
/// <summary>
/// BatchJob_GetRMMMailResultFromOMI 的摘要描述
/// 每日收OMI BILL平台不合作通知函MAIL發送失敗清單
/// RC單：RQ-2020-021027-001
/// 建立日期：2020/08/03
...
public class BatchJob_GetRMMMailResultFromOMI : Quartz.IJob
{
    ...
    protected string _MailTitle = "接收Omi Bill平台-不合作通知函名單回覆檔 批次：";
    ...
    public void Execute(JobExecutionContext context)
    {
        ...
        RMMMailResultFromOMIService _RMMMailResultFromOMIService = new RMMMailResultFromOMIService(JobID);
        ...
        try
        {
            ...
            if (isContinue)
            {
                ...
                _ExecDate = DateTime.Now.AddDays(-1).ToString("yyyyMMdd").Trim();//剔退檔內容為前一天的剔退內容
                ...
                DownloadFileAndInsertTable(JobID, _RMMMailResultFromOMIService, date, out total);
            }
        }
        catch (Exception ex)
        {
            ...
            // 發送 Email
            _RMMMailResultFromOMIService.SendMail(_MailTitle + " 失敗！總筆數：" + total, _MailTitle + fileName + " 失敗！總筆數：" + total + "，發生錯誤：" + ex.Message, "失敗", this.StartTime);
        }
        finally
        {
            // 20200031-CSIP EOS Ares Luke 修改日期:2021/03/12 修改說明:業務需求排程結束清空FilInfo參數
    ...
    private string DownloadFileAndInsertTable(string JobID, RMMMailResultFromOMIService _RMMService, string date, out int total)
    {
        ...
        string fileName = _RMMService.DownloadFromFTP(date, localPath, "TXT", ref errorDownloadMsg, ref isDownloadSucc);

        if (isDownloadSucc)//若檔案下載成功才將資料寫入DataTable中
        {
            // 取檔案內容
            dat = _RMMService.GetFileToDataTable(localPath, fileName, out errorMsg);
        }
        ...
        if (errorMsg == "")
        {
            isUpdateOK = _RMMService.SetDataTableToDB(dat, _ExecDate, ref errorMsg, ref total, ref SendFailCorp);//20200820-10月RC-email失敗的統編寫在批次通知信函內
        }
        ...
        {   
            if (!isDownloadSucc)
            {
                ...
                JobHelper.Write(JobID, _MailTitle + fileName + " 失敗！總筆數:" + total + "，失敗原因：未收到OMI BILL平台-不合作通知函名單回覆檔", LogState.Info);
                // 發送 Email
                _RMMService.SendMail(_MailTitle + "失敗！未收到檔案", _MailTitle + fileName + " 失敗！失敗原因：未收到OMI BILL平台-不合作通知函名單回覆檔", "失敗", this.StartTime);
            }
            else if (errorMsg.Trim().Equals("資料為空檔"))
            {
                ...
                _RMMService.SendMail(_MailTitle + "成功，無筆數", _MailTitle + fileName + " 成功！無筆數", "成功", this.StartTime);
            }
            else
            {
                InsertBatchLog(JobID, total, 0, "F", errorMsg);
                ...
                // 發送 Email
                _RMMService.SendMail(_MailTitle + "失敗！，總筆數：" + total, _MailTitle + fileName + " 失敗！總筆數：" + total + "，失敗原因：" + errorMsg, "失敗", this.StartTime);
            }
        }
        else
        {
            ...
            JobHelper.Write(JobID, "接收Omi Bill平台-不合作通知函名單回覆檔 批次：" + fileName + " 成功！總筆數:" + total + "，匯入成功共" + total + "筆，匯入失敗O筆", LogState.Info);

            // 發送 Email
            //20200820-10月RC-email失敗的統編寫在批次通知信函內
            _RMMService.SendMail(_MailTitle + "成功！總筆數：" + total, _MailTitle + fileName + " 成功！總筆數：" + total + "，匯入成功共" + total + "筆，匯入失敗O筆 ，email寄送失敗統編：" + SendFailCorp, "成功", this.StartTime);
        }
