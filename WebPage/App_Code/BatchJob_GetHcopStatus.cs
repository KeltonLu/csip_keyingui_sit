...
/// <summary>
/// BatchJob_GetHcopStatus 的摘要描述
/// 每日收主機的最新的總公司狀態檔
/// 預計運行模式：每日收檔後，先truncate 後Insert
/// </summary>
public class BatchJob_GetHcopStatus : Quartz.IJob
{
    ...
    protected string _MailTitle = "最新總公司狀態檔 批次：";
    ...
    public void Execute(JobExecutionContext context)
    {
        ...
        MainFrameHCOPStatusService _MainFrameHCOPStatusService = new MainFrameHCOPStatusService(JobID);

        try
        {
            JobHelper.Write(JobID, "*********** " + JobID + " 最新總公司狀態檔 批次 START ************** ", LogState.Info);
            ...
            if (isContinue)
            {
                ...
                DownloadFileAndInsertTable(JobID, _MainFrameHCOPStatusService, date, out total);
            }
        }
        catch (Exception ex)
        {
            JobHelper.Write(JobID, "[FAIL]  最新總公司狀態檔 批次" + fileName + " 發生錯誤：" + ex.Message);
            ...
            // 發送 Email
            _MainFrameHCOPStatusService.SendMail(_MailTitle + " 失敗！總筆數：" + total, _MailTitle + fileName + " 失敗！總筆數：" + total + "，發生錯誤：" + ex.Message, "失敗", this.StartTime);
        }
        finally
        {
            // 20200031-CSIP EOS Ares Luke 修改日期:2021/03/12 修改說明:業務需求排程結束清空FilInfo參數
            ...
            JobHelper.Write(JobID, "  最新總公司狀態檔 批次 Job 結束！", LogState.Info);
    ...
    private string DownloadFileAndInsertTable(string JobID, MainFrameHCOPStatusService _RLService, string date, out int total)
    {
        ...
        string fileName = _RLService.DownloadFromFTP(date, localPath, "txt", ref errorCtlMsg ,ref isDownloadSucc);
        ...
        if (errorMsg == "")
        {
            ...
            //先刪除
            isDelOK = CSIPKeyInGUI.BusinessRules_new.BRAML_HCOP_STATUS.Truncate(ref errorMsg);
        ...
        if (errorMsg != "" || !isInsertOK || !isDownloadSucc)
        {
            ...
            if (!isDownloadSucc)
            {
                JobHelper.Write(JobID, "最新總公司狀態檔 批次：" + fileName + " 失敗！總筆數:" + total + "，失敗原因：未收到總公司狀態檔");
                // 發送 Email
                _RLService.SendMail(_MailTitle + "失敗！未收到檔案", _MailTitle + fileName + " 失敗！總筆數：" + total + "，失敗原因：未收到總公司狀態檔", "失敗", this.StartTime);
            }
            else
            {
                JobHelper.Write(JobID, "最新總公司狀態檔 批次：" + fileName + " 失敗！總筆數:" + total + errorMsg);
                // 發送 Email
        ...
        else
        {
            ...
            JobHelper.Write(JobID, "最新總公司狀態檔 批次：" + fileName + " 成功！總筆數:" + total + "，匯入成功共" + total + "筆，匯入失敗O筆", LogState.Info);
