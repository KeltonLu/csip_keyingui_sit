...
/// <summary>
/// BatchJob_GetMainFrameMFAF 的摘要描述
/// 每日收主機的MFAF設定檔
/// 預計運行模式：每日收檔後，先truncate 後Insert
/// </summary>
public class BatchJob_GetMainFrameMFAF : Quartz.IJob
{
    ...
    protected string _MailTitle = "主機MFA設定檔批次：";
    ...
    public void Execute(JobExecutionContext context)
    {
        ...
        MainFrameMFAFService _MainFrameMFAFService = new MainFrameMFAFService(JobID);

        try
        {
            JobHelper.Write(JobID, "*********** " + JobID + " 取得【主機MFAF設定檔】批次 START ************** ", LogState.Info);
            ...
            if (isContinue)
            {
                ...
                DownloadFileAndInsertTable(JobID, _MainFrameMFAFService, date, out total);
            }
        }
        catch (Exception ex)
        {
            JobHelper.Write(JobID, "[FAIL]  取得【主機MFAF設定檔】批次 " + fileName + " 發生錯誤：" + ex.Message);
            ...
            // 發送 Email
            _MainFrameMFAFService.SendMail(_MailTitle + fileName + " 失敗！", "總筆數:" + total, ex.Message, this.StartTime);
        }
        finally
        {
            // 20200031-CSIP EOS Ares Luke 修改日期:2021/03/12 修改說明:業務需求排程結束清空FilInfo參數
            ...
            JobHelper.Write(JobID, "  取得【主機MFAF設定檔】批次 Job 結束！", LogState.Info);
    ...
    private string DownloadFileAndInsertTable(string JobID, MainFrameMFAFService _RLService, string date, out int total)
    {
        ...
        if (errorMsg == "")
        {
            bool isDelOK = false;
            //先刪除
            isDelOK = CSIPKeyInGUI.BusinessRules_new.BRAML_HQ_MFAF.Truncate(ref errorMsg);
        ...
        if (errorMsg != "" || !isInsertOK || !isDownloadSucc)
        {
            ...
            JobHelper.Write(JobID, "取得【主機MFAF設定檔】批次:" + fileName + " 失敗！總筆數:" + total + errorMsg);

            if (!isDownloadSucc)
            {
                // 發送 Email
                _RLService.SendMail(_MailTitle + " 失敗！未收到檔案", _MailTitle + fileName + " 失敗！未收到檔案", errorMsg, this.StartTime);
            }
            else
            {
                // 發送 Email
        ...
        else
        {
            ...
            JobHelper.Write(JobID, "取得【主機MFAF設定檔】批次：" + fileName + " 成功！總筆數：" + total + "，匯入成功共" + total + "筆，匯入失敗O筆", LogState.Info);
