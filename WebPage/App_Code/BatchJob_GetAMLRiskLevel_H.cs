...
/// <summary>
/// BatchJob_GetAMLRiskLevel_H 的摘要描述
/// 每日收AML下傳的高風險國家清單
/// 預計運行模式：每日收檔後，先tDelete 後Insert
/// </summary>
public class BatchJob_GetAMLRiskLevel_H : Quartz.IJob
{
    ...
    protected string _MailTitle = "高風險國家清單(H)檔 批次：";
    ...
    public void Execute(JobExecutionContext context)
    {
        ...
        try
        {
            JobHelper.Write(JobID, "*********** " + JobID + " 高風險國家清單(H)檔 批次 START ************** ", LogState.Info);
        ...
        {
            JobHelper.Write(JobID, "[FAIL]  高風險國家清單(H)檔 批次：" + fileName + " 發生錯誤：" + ex.Message);
        ...
        finally
        {
            ...
            JobHelper.Write(JobID, "  高風險國家清單(H)檔 批次 Job 結束！", LogState.Info);
    ...
    private string DownloadFileAndInsertTable(string JobID, AMLRiskLevelSettingService _RLService, string date, out int total)
    {
        ...
        {
            //先刪除, 測試時先mark
            CSIPKeyInGUI.BusinessRules_new.BRPostOffice_CodeType.DeleteCodeTypeByType("12");
            // 寫入資料庫
        ...
        if (errorMsg != "" || !isInsertOK || !isDownloadSucc)
        {
            if (!isDownloadSucc)
            {
                ...
                JobHelper.Write(JobID, "高風險國家清單(H)檔 批次：" + fileName + " 失敗！總筆數：" + total + "，未收到高風險國家清單(H)檔");

                // 發送 Email
                _RLService.SendMail(_MailTitle + " 失敗！未收到檔案", _MailTitle + fileName + " 失敗！總筆數：" + total + "，未收到高風險國家清單(H)檔", "失敗", this.StartTime);
            }
            else
            {
                InsertBatchLog(JobID, total, 0, "F", errorMsg);

                JobHelper.Write(JobID, "高風險國家清單(H)檔 批次：" + fileName + " 失敗！總筆數：" + total + "，失敗原因：" + errorMsg);
        ...
        else
        {
            ...
            JobHelper.Write(JobID, "高風險國家清單(H)檔 批次：" + fileName + " 成功！總筆數:" + total + "，匯入成功共" + total + "筆，匯入失敗O筆", LogState.Info);
