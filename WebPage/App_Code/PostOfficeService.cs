...
public class PostOfficeService
{
    ...
    public PostOfficeService(string jobID)
    {
        this.jobID = jobID;
    }
    ...
    {
        return BRFORM_COLUMN.GetSendToPostOfficeData(endTime);
    }
    ...
    public bool UploadFileToMFTP(string jobID, DataTable localFileInfo, string localPath, string fileName)
    {
        try
        {
            ...
            bool isGet = BRFORM_COLUMN.GetFTPInfo(jobID, ref ftpIP, ref ftpId, ref ftpPwd, ref ftpPath, ref ZipPwd);
    ...
    public DataTable GetReplyFileStream(string filePath, out DataTable activeData)
    {
        ...
        {
            Logging.Log(this.jobID + ex.ToString(), LogState.Error, LogLayer.Util);
        }
        finally
        {
            sr.Close();
    ...
    /// 修改紀錄：20211021 不異動Email、電子賬單 By Ares Stanley
    /// <summary>
    /// 回寫回覆檔相關 Table
    /// </summary>
                ...
                if (masterOK)
                {
                    foreach (DataRow row in replyData.Rows)
                    {
                        if (row["DataType"].ToString().Trim() == "1")
                        {
                            ...
                            postOfficeTemp = BRFORM_COLUMN.GetReceiveNumber(row, true);
                            if (postOfficeTemp.Rows.Count > 0)
                            {
    ...
    /// 修改紀錄：20211021 不異動電子帳單 By Ares Stanley
    /// <summary>
    /// 上送主機
    /// </summary>
