...
//*Ares Luke          2021/04/13         20200031-CSIP EOS       調整Log層級
//*******************************************************************
...
/// <summary>
/// MailResultFromOMIService 的摘要描述
/// </summary>
public class MailResultFromOMIService
{
    private string JobID = string.Empty;
    protected JobHelper JobHelper = new JobHelper();

    public MailResultFromOMIService(string JobID)
    {
        this.JobID = JobID;
    }
    ...
    public bool SetDataTableToDB(DataTable sourceDat, string _ExecDate, ref string errorMsg, ref int Count_0049, ref string _returnCorp)
    {
        ...
        try
        {
            ...
            //mailLog = CSIPKeyInGUI.BusinessRules_new.BRAML_AUDITMAILLOG.GetMailLogData(_ExecDate.Trim());
            mailLog = CSIPKeyInGUI.BusinessRules_new.BRAML_AUDITMAILLOG.GetMailLogData(_ExecDate.Trim(), "A");
            ...
            foreach (DataRow row in sourceDat.Rows)
            {
                ...
                if (o_dr.Length > 0)
                {
                    ...
                    isUpdateData = CSIPKeyInGUI.BusinessRules_new.BRAML_AUDITMAILLOG.Update(row["CORP_NO"].ToString().Trim(), _ExecDate.Trim(), "A", ref errorMsg);
                    //20200804-RQ-2020-021027-001 調整寫入NOTELOG的TYPE
                    CSIPCommonModel.BusinessRules.BRFORM_COLUMN.AML_NOTELOG(o_dr[0]["CASE_NO"].ToString(), row["CORP_NO"].ToString(), "", "MERCHANTAMLRESULT", "傳送定期審查地址條(Email)失敗");
                    Count_0049++;
                }
            }
            ...
            JobHelper.Write(this.JobID, "更新AML_AUDITMAILLOG 資料庫結束！結果： " + isUpdateData, LogState.Info);

            if (isUpdateData)
            {
                JobHelper.Write(this.JobID, "更新AML_HQ_WORK的AddressLabelflag 資料開始！ ", LogState.Info);

                //再以LOG檔的統編將AML_HQ_WORK的AddressLabelflag更新成NULL，以利發送紙本定審通知函
                isUpdateData = CSIPKeyInGUI.BusinessRules_new.BRAML_AUDITMAILLOG.UpdateHQ_WORK(_ExecDate.Trim(), ref errorMsg);
            }

            JobHelper.Write(this.JobID, "更新AML_HQ_WORK的AddressLabelflag 資料結束！結果：" + isUpdateData , isUpdateData ? LogState.Info : LogState.Error);
        }
        catch (Exception ex)
        {
            errorMsg = ex.Message;
