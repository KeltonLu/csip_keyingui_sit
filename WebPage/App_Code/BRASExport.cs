...
//*  功能說明：人工簽單-檔案匯出
//*  創建日期：2014/8/11
//*  修改紀錄：2020/12/14_Ares_Stanley-新增NPOI; 2021/04/01_Ares_Stanley-移除MicrosoftExcel
...
public class BRASExport : CSIPCommonModel.BusinessRules.BRBase<EntityM_PROPERTY_CODE>
{
    ...
    //新增 匯出至ACQ前先查詢簽單筆數及金額，確認OK再送出
    //2021/03/09_Ares_Stanley-DB名稱改為變數
    private const string SearchTotalApply_AMT = @"  SELECT  
            SUM(a.Receive_Total_Count) AS Receive_Total_Count, SUM(a.Receive_Total_AMT) AS Receive_Total_AMT 
            FROM (
	            SELECT Batch_Date, Receive_Batch, Batch_NO, Shop_ID, Sign_Type, Receive_Total_Count, Receive_Total_AMT, Process_Flag
	            FROM [{0}].[dbo].[Artificial_Signing_Batch_Data]
    ...
    private const string Sel_Count = @" SELECT COUNT(*),SUM([Receive_Total_Count]),SUM([Receive_Total_AMT]) FROM Artificial_Signing_Batch_Data 
            WHERE Batch_Date=@Batch_Date AND Receive_Batch=@Receive_Batch AND Sign_Type=@Sign_Type ";

    private const string CHECK_Before_Upload = @" SELECT COUNT(*) FROM Artificial_Signing_Batch_Data 
            WHERE Batch_Date = @Batch_Date AND Receive_Batch = @Receive_Batch AND Sign_Type = @Sign_Type AND Process_Flag = @Process_Flag";

    #endregion


    public static DataTable CheckBeforeUpload(string strBatchDate, string strReceiveBatch, string strSignType, string strProcessFlag, ref string strMsgID)
    {
        try
        {
            ...
            sqlcmSearchData.CommandText = CHECK_Before_Upload;
            //* 編批日期
            SqlParameter parmBatchDate = new SqlParameter("@Batch_Date", strBatchDate.Replace("/", ""));
            sqlcmSearchData.Parameters.Add(parmBatchDate);
            //* 收件批次
            SqlParameter parmReceiveBatch = new SqlParameter("@Receive_Batch", strReceiveBatch);
            sqlcmSearchData.Parameters.Add(parmReceiveBatch);
            //* 簽單類別
            SqlParameter parmSignType = new SqlParameter("@Sign_Type", strSignType);
            sqlcmSearchData.Parameters.Add(parmSignType);
            //* 處理註記
            SqlParameter parmProcessFlag = new SqlParameter("@Process_Flag", strProcessFlag);
            sqlcmSearchData.Parameters.Add(parmProcessFlag);
        ...
        {
            CSIPKeyInGUI.BusinessRules.BRASErrKeyin.SaveLog(exp);
            strMsgID = "01_01070200_001";
            return null;
        }
    ...
    /// <summary>
    /// 匯出至ACQ前先查詢簽單筆數及金額，確認OK再送出
    /// </summary>
    /// <param name="strBatch_Date"></param>
    /// <param name="strReceive_Batch"></param>
    /// <returns></returns>
    public static DataTable SearchExportCheck(string strBatch_Date, string strReceive_Batch, ref string strMsgID) {
             ...
             try
        {
            ...
            sqlcmSearchData.CommandType = CommandType.Text;
            //2021/03/09_Ares_Stanley-DB名稱改為變數
            sqlcmSearchData.CommandText = string.Format(SearchTotalApply_AMT, UtilHelper.GetAppSettings("DB_KeyinGUI"));
            //* 編批日期
            ...
            //* 查詢數據
            DataSet dstSearchData = BRASExport.SearchOnDataSet(sqlcmSearchData,ref strMsgID);
            if (null == dstSearchData)  //* 查詢數據失敗
            {
    ...
    /// <summary>
    /// 產生請款簽單明細表Excel
    /// 修改紀錄:2020/12/23_Ares_Stanley-修改報表產出方式為NPOI; 2021/01/07_Ares_Stanley-移除舊excel function
    /// </summary>
    /// <param name="strBatch_Date">編批日期</param>
    /// <param name="strReceive_Batch">收件批次</param>
    ...
    /// <summary>
    /// 產生一般簽單高收檔 或 分期簽單高收檔
    /// 修改日期：2021/01/05_Ares_Stanley-變更報表產出方式為NPOI
    /// </summary>
    /// <param name="strBatch_Date">編批日期</param>
    /// <param name="strReceive_Batch">收件批次</param>
    ...
    /// <param name="strBatch_Date"></param>
    /// <param name="strReceive_Batch"></param>
    /// <param name="strSign_Type"></param>
    /// <param name="strPathFile"></param>
    /// <param name="strMsgID"></param>
    /// <returns></returns>
    /// 修改紀錄:2021/04/01_Ares_Stanley-註解舊版ExportAS_ACQNEW
    #region 舊CODE
    //public static bool ExportAS_ACQNEW(string strBatch_Date, string strReceive_Batch, string strSign_Type, ref string strPathFile, ref string strMsgID  )
    //{
    //    #endregion
    //    //解決"格式太舊或是類型程式庫無效。 (Exception from HRESULT: 0x80028018 (TYPE_E_INVDATAREAD))"問題用
    //    System.Globalization.CultureInfo oldCI = System.Threading.Thread.CurrentThread.CurrentCulture;
    ...
    //            strFileName = "P" + strBatch_Date_TW + strReceive_Batch + "A4";
    //        }
    //        //Talas 20191118 依參數調整存成XLS 還是TAB分隔的TXT 

    //        strPathFile = strPathFile + @"\" + strFileName + "_" + DateTime.Now.ToString("yyyyMMddHHmmss") + ".txt";
    //        //     sheet.SaveAs(strPathFile, Microsoft.Office.Interop.Excel.XlFileFormat.xlCSV);
    //        sheet.SaveAs(strPathFile, Microsoft.Office.Interop.Excel.XlFileFormat.xlCSV, Missing.Value, Missing.Value, Missing.Value, Missing.Value, Missing.Value, Missing.Value, Missing.Value, Missing.Value);
    ...
    /// <summary>
    /// 建立日期:2021/02/18_Ares_Stanley-改寫為NPOI方式產出
    /// </summary>
    /// <param name="strBatch_Date"></param>
    /// <param name="strReceive_Batch"></param>
    /// <param name="strSign_Type"></param>
    ...
    /// <param name="strMsgID"></param>
    /// <returns></returns>
    public static bool ExportAS_ACQNEW_NPOI(string strBatch_Date, string strReceive_Batch, string strSign_Type, ref string strPathFile, ref string strMsgID)
    {
        try
        {
            //* 檢查目錄 
            ...
            string strExcelPathFile = AppDomain.CurrentDomain.BaseDirectory + UtilHelper.GetAppSettings("ReportTemplate") + "ACQNEW_TEMP.xls";
            ...
            int intRowIndexInSheet = 0;
            #region 文字格式-千分位
            HSSFCellStyle thouFormat = (HSSFCellStyle)wb.CreateCellStyle(); //建立文字格式
            thouFormat.VerticalAlignment = VerticalAlignment.Center; //垂直置中
            thouFormat.Alignment = HorizontalAlignment.Center; //水平置中
            thouFormat.DataFormat = HSSFDataFormat.GetBuiltinFormat("#,##0"); //將儲存格內容設定為文字

            HSSFFont thouFont = (HSSFFont)wb.CreateFont(); //建立文字樣式
            thouFont.FontHeightInPoints = 12; //字體大小
            thouFont.FontName = "新細明體"; //字型
            //contentFont.Boldweight = (short)NPOI.SS.UserModel.FontBoldWeight.Bold; //粗體
            thouFormat.SetFont(thouFont); //設定儲存格的文字樣式
            #endregion
            ...
            for (int i = 0; i < iTotalPage; i++)
            {
                ...
                for (int intLoop = 0; intLoop < 31; intLoop++)
                {
                    ...
                    {
                        sheet.GetRow(sheet.LastRowNum).GetCell(0).SetCellValue("合計");
                        sheet.GetRow(sheet.LastRowNum).GetCell(1).SetCellValue("");
                        sheet.GetRow(sheet.LastRowNum).GetCell(2).SetCellValue("");
                        sheet.GetRow(sheet.LastRowNum).GetCell(3).SetCellValue(sumReceive_Total_Count.ToString("N0"));
                        sheet.GetRow(sheet.LastRowNum).GetCell(4).SetCellValue(sumReceive_Total_AMT.ToString("N0"));
                        sheet.GetRow(sheet.LastRowNum).GetCell(5).SetCellValue(sumAdjust_Count.ToString("N0"));
                        sheet.GetRow(sheet.LastRowNum).GetCell(6).SetCellValue(sumAdjust_AMT.ToString("N0"));
                        sheet.GetRow(sheet.LastRowNum).GetCell(7).SetCellValue(sumKeyin_Reject_Count_All.ToString("N0"));
                        sheet.GetRow(sheet.LastRowNum).GetCell(8).SetCellValue(sumKeyin_Reject_AMT_All.ToString("N0"));
                        sheet.GetRow(sheet.LastRowNum).GetCell(9).SetCellValue("");
                        sheet.GetRow(sheet.LastRowNum).GetCell(10).SetCellValue("");
                        break;
                    }
                    ...
                    if (iDataIndex > 0 && intLoop > 0)
                    {
                        if (iDataIndex >= dt.Rows.Count)
                        {
                            sheet.GetRow(sheet.LastRowNum).GetCell(0).SetCellValue(dt.Rows[iDataIndex - 1]["Page"].ToString());
                            for (int x = 1; x < 11; x++)
                            {
                                sheet.GetRow(sheet.LastRowNum).GetCell(x).SetCellValue("");
                            }
                            continue;
                        }
                        if (dt.Rows[iDataIndex]["Page"].ToString() != dt.Rows[iDataIndex - 1]["Page"].ToString())
                    ...
                    sheet.GetRow(sheet.LastRowNum).GetCell(2).SetCellValue(dt.Rows[iDataIndex]["Shop_ID"].ToString());
                    //* 請款筆數
                    sheet.GetRow(sheet.LastRowNum).GetCell(3).SetCellValue(dt.Rows[iDataIndex]["Receive_Total_Count"].ToString().Equals("") ? "" : Convert.ToDecimal(dt.Rows[iDataIndex]["Receive_Total_Count"].ToString()).ToString("N0"));
                    //* 請款金額
                    sheet.GetRow(sheet.LastRowNum).GetCell(4).SetCellValue(dt.Rows[iDataIndex]["Receive_Total_AMT"].ToString().Equals("") ? "" : Convert.ToInt32(dt.Rows[iDataIndex]["Receive_Total_AMT"].ToString()).ToString("N0"));
                    //* 調整筆數
                    sheet.GetRow(sheet.LastRowNum).GetCell(5).SetCellValue(dt.Rows[iDataIndex]["Adjust_Count"].ToString().Equals("") ? "" : Convert.ToDecimal(dt.Rows[iDataIndex]["Adjust_Count"].ToString()).ToString("N0"));
                    //* 調整金額
                    sheet.GetRow(sheet.LastRowNum).GetCell(6).SetCellValue(dt.Rows[iDataIndex]["Adjust_AMT"].ToString().Equals("") ? "" : Convert.ToInt32(dt.Rows[iDataIndex]["Adjust_AMT"].ToString()).ToString("N0"));
                    //* 剔退筆數
                    sheet.GetRow(sheet.LastRowNum).GetCell(7).SetCellValue(dt.Rows[iDataIndex]["Keyin_Reject_Count_All"].ToString().Equals("") ? "" : Convert.ToDecimal(dt.Rows[iDataIndex]["Keyin_Reject_Count_All"].ToString()).ToString("N0"));
                    //* 剔退金額
                    sheet.GetRow(sheet.LastRowNum).GetCell(8).SetCellValue(dt.Rows[iDataIndex]["Keyin_Reject_AMT_All"].ToString().Equals("") ? "" : Convert.ToInt32(dt.Rows[iDataIndex]["Keyin_Reject_AMT_All"].ToString()).ToString("N0"));
                    //* 原因碼(Substring去掉第一個,)
                    //20141017  ,改以空格取代
                    sheet.GetRow(sheet.LastRowNum).GetCell(9).SetCellValue(dt.Rows[iDataIndex]["Reject_Reason"].ToString().Length > 0 ? dt.Rows[iDataIndex]["Reject_Reason"].ToString().Substring(1).Replace(',', ' ') : "");
                    //* 正確商店代號
                    ...
                    //* 計算合計列資料
                    sumReceive_Total_Count += sheet.GetRow(sheet.LastRowNum).GetCell(3).StringCellValue.Equals("") ? 0 : Convert.ToInt32(sheet.GetRow(sheet.LastRowNum).GetCell(3).StringCellValue.Replace(",",""));
                    sumReceive_Total_AMT += sheet.GetRow(sheet.LastRowNum).GetCell(4).StringCellValue.Equals("") ? 0 : Convert.ToDecimal(sheet.GetRow(sheet.LastRowNum).GetCell(4).StringCellValue.Replace(",", ""));
                    sumAdjust_Count += sheet.GetRow(sheet.LastRowNum).GetCell(5).StringCellValue.Equals("") ? 0 : Convert.ToInt32(sheet.GetRow(sheet.LastRowNum).GetCell(5).StringCellValue.Replace(",", ""));
                    sumAdjust_AMT += sheet.GetRow(sheet.LastRowNum).GetCell(6).StringCellValue.Equals("") ? 0 : Convert.ToDecimal(sheet.GetRow(sheet.LastRowNum).GetCell(6).StringCellValue.Replace(",", ""));
                    sumKeyin_Reject_Count_All += sheet.GetRow(sheet.LastRowNum).GetCell(7).StringCellValue.Equals("") ? 0 : Convert.ToInt32(sheet.GetRow(sheet.LastRowNum).GetCell(7).StringCellValue.Replace(",", ""));
                    sumKeyin_Reject_AMT_All += sheet.GetRow(sheet.LastRowNum).GetCell(8).StringCellValue.Equals("") ? 0 : Convert.ToDecimal(sheet.GetRow(sheet.LastRowNum).GetCell(8).StringCellValue.Replace(",", ""));
                    iDataIndex++;
                }
                intRowIndexInSheet = (i * iPageRow) + intRowIndexStart;
                intRowIndexEnd = intRowIndexInSheet + iPageRow - 1;
            ...
            sheet.GetRow(sheet.LastRowNum).GetCell(3).SetCellValue(sumReceive_Total_Count_ALL.ToString("N0"));
            sheet.GetRow(sheet.LastRowNum).GetCell(4).SetCellValue(sumReceive_Total_AMT_ALL.ToString("N0"));
            sheet.GetRow(sheet.LastRowNum).GetCell(5).SetCellValue(sumAdjust_Count_ALL.ToString("N0"));
            sheet.GetRow(sheet.LastRowNum).GetCell(6).SetCellValue(sumAdjust_AMT_ALL.ToString("N0"));
            sheet.GetRow(sheet.LastRowNum).GetCell(7).SetCellValue(sumKeyin_Reject_Count_All_ALL.ToString("N0"));
            sheet.GetRow(sheet.LastRowNum).GetCell(8).SetCellValue(sumKeyin_Reject_AMT_All_ALL.ToString("N0"));
            intRowIndexEnd += 2;
            for (int i = 0; i < 2; i++)
            {
                sheet.CreateRow(sheet.LastRowNum + 1);
            ...
            sheet.GetRow(sheet.LastRowNum).GetCell(3).SetCellValue("淨請款筆數 :");
            sheet.GetRow(sheet.LastRowNum).GetCell(4).SetCellValue((sumNet_Count - sumKeyin_Reject_Count_All_ALL).ToString("N0"));
            ...
            sheet.GetRow(sheet.LastRowNum).GetCell(3).SetCellValue("淨請款金額 :");
            sheet.GetRow(sheet.LastRowNum).GetCell(4).SetCellValue((sumNet_AMT - sumKeyin_Reject_AMT_All_ALL).ToString("N0"));
            // 含千分位符號的資料加上引號
            for (int row = 2; row < sheet.LastRowNum + 1; row++)
            {
                for (int col = 3; col < 9; col++)
                {
                    if (!sheet.GetRow(row).GetCell(col).StringCellValue.Equals(""))
                    {
                        if (sheet.GetRow(row).GetCell(col).StringCellValue.Contains(","))
                        {
                            sheet.GetRow(row).GetCell(col).SetCellValue("\"" + sheet.GetRow(row).GetCell(col).StringCellValue + "\"");
                        }
                    }
                }
            }
    ...
    /// <summary>
    /// 產生重覆件報表Excel
    /// 修改紀錄:2021/02/22_Ares_Stanley-註解舊版程式碼
    /// </summary>
    /// <param name="strPathFile">服務器端生成的Excel文檔路徑</param>
    /// <param name="strMsgID">返回的錯誤ID號</param>
    /// <returns>Excel生成成功標示：True--成功；False--失敗</returns>
    ...
    public static void DropFiles(string sPath, string expDay)
    {
        int delDay = 7;
    ...
    /// <param name="strReceive_Batch"></param>
    /// <returns></returns>
    public static DataTable confirmCount(string strBatch_Date, string strReceive_Batch, string strSign_Type, ref string strMsgID)
    {
        ...
        try
        {
            ...
            SqlCommand sqlcmSearchData = new SqlCommand();
            sqlcmSearchData.CommandType = CommandType.Text;
            sqlcmSearchData.CommandText = Sel_Count;
            //* 編批日期
            SqlParameter parmBatchDate = new SqlParameter("@Batch_Date", strBatch_Date.Replace("/", ""));
            sqlcmSearchData.Parameters.Add(parmBatchDate);
            //* 收件批次
