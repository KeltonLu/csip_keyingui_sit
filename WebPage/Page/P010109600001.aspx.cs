...
public partial class P010109600001 : PageBase
{
    #region 變數區
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            if (sessionOBJ != null)
            {
                ...
                if (!bHaveData)
                {
                    string NavigateUrl = "P010109060001.aspx";
                    string urlString = @"alert('查無資料');location.href='" + NavigateUrl + "';";
                    base.sbRegScript.Append(urlString);
                }               
            }
        ...
        try
        {
            if (sessionOBJ != null)
            {
                ...
                if (hqObj != null)
                {
                    ...
                    //{
                    //    string[] incomes = hqObj.HCOP_INCOME_SOURCE.Split(',');
                    //    List<string> incomeNames = new List<string>();
                    //    foreach (string income in incomes)
                    //    {
                    //        DataTable dtIncome = new DataTable();
                    //        //主要收入來源 Type=17
                    //        dtIncome = BRPostOffice_CodeType.GetCodeTypeByCodeID("17", income);
                    //        if (dtIncome != null && dtIncome.Rows.Count > 0)
                    //        {
                    //            incomeNames.Add(dtIncome.Rows[0]["CODE_NAME"].ToString());
                    //        }
                    //        else
                    //        {
                    ...
                    //        dtIncome.Dispose();
                    //    }
                    //    HQlblHCOP_INCOME_SOURCE.Text = string.Join(",", incomeNames);
                    //}
                    if (!string.IsNullOrEmpty(hqObj.HCOP_INCOME_SOURCE))
                    {
                    ...
                    //行業編號 Type=3 //20211025_Ares_Jack_高風險行業別 Type = 11
        ...
        try
        {
            if (sessionOBJ != null)
            {
                ...
                if (sccdObj != null)
                {
                    ...
                    hidHQ_SCDD_NameCheck_Item.Value = sccdObj.NameCheck_Item;//20211025_Ares_Jack_取消註解 名單掃描紀錄
