<%@ Page Language="C#" AutoEventWireup="true" CodeFile="P010213030001.aspx.cs" Inherits="P010213030001" %>
...
function CheckInput()
{
    ...
    // 車號可不輸入，若有輸入則主機會以此條件過濾；A. 車號須向左靠、中間以"-"分隔，B. 英數字皆為半形、中間去掉空白
    var lprNO = $('#txtLPR_NO').val().Trim();
    var keyInDateS = $('#dpKeyInDateS_foo').val().Trim();
    var keyInDateE = $('#dpKeyInDateE_foo').val().Trim();
	...
	else
	{
	    ...
	    if(keyInDateE == '')
	    {
	        ...
	        keyInDateE = year + '/' + (month < 10 ? '0' : '') + month + '/' + (day < 10 ? '0' : '') + day;
	        $('#dpKeyInDateE_foo').val(keyInDateE);
	    }
	    
	    // 檢查區間最長為6個月
	    if(!CheckDateRange(keyInDateS, keyInDateE))
	        return false;
	}
}
...
function CheckDateRange(dateS, dateE)
{
    var parts = dateE.split('/');
    var de = new Date(parts[0], parts[1] - 1, parts[2]);
    parts = dateS.split('/');
    var ds = new Date(parts[0], parts[1] - 1, parts[2]);
    
    var maxyear = parseInt(parts[0]);
    var maxMonth = parseInt(parts[1]) + 6;
    // dd/MM/yyyy
    var maxDate = (maxMonth > 12 ? maxMonth - 12 : maxMonth) + '/' + parts[2] + '/' + (maxMonth > 12 ? maxyear + 1 : maxyear);
                            ...
                            <td colspan="6">
                                <li>
                                    <cc1:CustLabel ID="lblTitle" runat="server" CurAlign="left" CurSymbol="£" FractionalDigit="2" 
                                        ...
                                        SetOmit="False" StickHeight="False" Width="200px" ShowID="01_02130300_001" />
                                </li>
                            </td>
                        </tr>
                        <tr class="trOdd">
                            <td style="width: 10%" align="right">
                                <cc1:CustLabel ID="lblACCT_ID" runat="server" CurAlign="left" CurSymbol="£" FractionalDigit="2" 
                                    ...
                                    SetOmit="False" StickHeight="False" ShowID="01_02130300_002" />
                            </td>
                            <td style="width: 15%">
                            ...
                            <td style="width: 10%" align="right">
                                <cc1:CustLabel ID="lblLPR_NO" runat="server" CurAlign="left" CurSymbol="£" FractionalDigit="2" 
                                    IsColon="True" IsCurrency="False" NeedDateFormat="False" NumBreak="0" NumOmit="0" SetBreak="False" 
                                    SetOmit="False" StickHeight="False" ShowID="01_02130300_003" />
                            </td>
                            <td style="width: 15%">
                            ...
                            <td style="width: 10%" align="right">
                                <cc1:CustLabel ID="lblAPPLY_NO" runat="server" CurAlign="left" CurSymbol="£" FractionalDigit="2" 
                                    IsColon="True" IsCurrency="False" NeedDateFormat="False" NumBreak="0" NumOmit="0" SetBreak="False" 
                                    SetOmit="False" StickHeight="False" ShowID="01_02130300_004" />
                            </td>
                            <td style="width: 40%">
                                <cc1:CustTextBox ID="txtAPPLY_NO" runat="server" MaxLength="16" checktype="num" BoxName="收件編號" />
                            </td>
                        </tr>
                        <tr class="trEven">
                            <td style="width: 10%" align="right">
                                <cc1:CustLabel ID="lblKeyInDate" runat="server" CurAlign="left" CurSymbol="£" FractionalDigit="2" 
                                    IsColon="True" IsCurrency="False" NeedDateFormat="False" NumBreak="0" NumOmit="0" SetBreak="False" 
                                    SetOmit="False" StickHeight="False" ShowID="01_02130300_005" />
                            </td>
                            <td style="width: 90%" colspan="5">
                                <cc1:DatePicker ID="dpKeyInDateS" runat="server"></cc1:DatePicker>
                                ~
                                <cc1:DatePicker ID="dpKeyInDateE" runat="server"></cc1:DatePicker>
                                <%--<cc1:CustTextBox ID="txtKeyInDateS" runat="server" MaxLength="8" checktype="num" BoxName="鍵檔日期S" />
                                ~
                                <cc1:CustTextBox ID="txtKeyInDateE" runat="server" MaxLength="8" checktype="num" BoxName="鍵檔日期E" />--%>
                                &nbsp;&nbsp;&nbsp;&nbsp;
                                <cc1:CustButton ID="btnSelect" CssClass="smallButton" runat="server" Width="40px"
                                    DisabledWhenSubmit="False" ShowID="01_02130300_006" OnClick="btnSelect_Click" 
                                ...
                                <td>
                                    <cc1:CustGridView ID="gvpbCase" runat="server" AllowSorting="True" PagerID="gpList"
                                        ...
                                        <Columns>
                                            ...
                                            <asp:BoundField DataField="TYPE"><itemstyle horizontalalign="Center" /></asp:BoundField><%--代繳種類--%>
                                            ...
                                            <asp:BoundField DataField="ACTION"><itemstyle horizontalalign="Center" /></asp:BoundField><%--異動碼--%>
                                            ...
                                            <asp:BoundField DataField="VENDOR_NAME"><itemstyle horizontalalign="Center" /></asp:BoundField><%--月租停車場名稱--%>
                                        </Columns>
                                    </cc1:CustGridView>
