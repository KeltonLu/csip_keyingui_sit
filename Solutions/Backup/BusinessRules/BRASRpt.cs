    ...
    {    
        ...
        @"SELECT CASE WHEN t.KeyinDate IS NULL THEN d.KeyinDate ELSE t.KeyinDate END KeyinDate, 
            ...
            FROM (
	            ...
	            LEFT JOIN [KeyinGUI].[dbo].[Artificial_Signing_Batch_Data] b ON a.Batch_Date = b.Batch_Date
            ...
            FULL OUTER JOIN (
	            ...
	            FROM [KeyinGUI].[dbo].[Artificial_Signing_Error]
        ...
        INNER JOIN (
	        ...
	        FROM (SELECT * FROM [KeyinGUI].[dbo].[Artificial_Signing_Error] WHERE Create_DateTime BETWEEN @SearchStart AND @SearchEnd) d
	        LEFT JOIN (
		        SELECT *
		        FROM [CSIP].[dbo].[M_PROPERTY_CODE]
        ...
        @"SELECT a.Batch_Date, (Convert(VARCHAR, a.Batch_NO) + ' / ' + a.Shop_ID) AS Compare, a.Card_No, 
            ...
            LEFT JOIN [KeyinGUI].[dbo].[Artificial_Signing_Primary] b 
                                ...
                                string strSearchEnd, ref string strMsgID)
        {
            try
            {
                ...
                switch (strProperty)    //* 業務項目
                {
                    case "rptAS_Work":
                        //* 人工簽單作業量報表
                        strSqlSearch = SEARCH_rptAS_Work;
                        break;
                    case "rptAS_ErrDetail":
                        //* 人工簽單鍵檔錯誤明細
                        strSqlSearch = SEARCH_rptAS_ErrDetail;
                        break;
                    case "rptAS_Capacity":
                        //* 人員鍵檔產能報表
                        strSqlSearch = SEARCH_rptAS_Capacity;
                        break;
                    case "rptAS_RejectDetail":
                        //* 人工簽單每日剔退明細
                        strSqlSearch = SEARCH_rptAS_RejectDetail;
                        break;
                    default:
                        break;
                }
                ...
                //* 查詢數據
                DataSet dstSearchData = BRASRpt.SearchOnDataSet(sqlcmSearchData);
