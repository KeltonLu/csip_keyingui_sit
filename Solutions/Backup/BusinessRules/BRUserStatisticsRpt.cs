...
namespace CSIPKeyInGUI.BusinessRules
{
    ...
    public class BRUserStatisticsRpt : CSIPCommonModel.BusinessRules.BRBase<EntityCUSTOMER_LOG>
    {
        ...
        private const string SEL_USER_STATISTICS = @"SELECT A.user_id user_id,B.USER_NAME " +
                ...
                "FROM AUTO_PAY C,(select distinct user_id,User_name from csip.dbo.M_USER) as  D " +
                "where C.mod_date between @SearchStart and @SearchEnd " +
                    "AND C.user_id = D.USER_ID " +
                "UNION " +
                "SELECT M.user_id user_id,N.USER_NAME " +
                "FROM Other_Bank_Temp M,(select distinct user_id,User_name from csip.dbo.M_USER) as N " +
                ...
                "FROM ReIssue_Card E,(select distinct user_id,User_name from csip.dbo.M_USER) as  F " +
                "where E.mod_date between @SearchStart and @SearchEnd " +
                    "AND E.user_id = F.USER_ID " +
                "UNION " +
                "SELECT G.user_id user_id,H.USER_NAME " +
                "FROM Change_BLK G,(select distinct user_id,User_name from csip.dbo.M_USER) as  H " +
                ...
                "FROM Balance_Trans q,(select distinct user_id,User_name from csip.dbo.M_USER) as  r " +
                "where q.Trans_Date between @SearchStart and @SearchEnd " +
                    "AND q.Modify_User = r.USER_ID " +
                "UNION " +
                "SELECT s.user_id user_id,t.USER_NAME " +
                "FROM iPASS s,(select distinct user_id,User_name from csip.dbo.M_USER) as t " +
                                ...
                                string strSearchEnd, ref string strMsgID)
        {
            try
            {
                ...
                //* 執行查詢SQL語句
                DataSet dstSearchResult = BRUserStatisticsRpt.SearchOnDataSet(sqlcmdSearch);
                ...
                for (int intLoop = 0; intLoop < dstSearchResult.Tables[0].Rows.Count; intLoop++)
                {
                    ...
                    //* 執行查詢SQL語句
                    dstResult = BRUserStatisticsRpt.SearchOnDataSet(sqlcmdSearch);
                    if (null == dstResult)
                    {
                        strMsgID = "01_03030000_002";
                        return null;
                            ...
                            out ReportDocument rptResult, ref string strMsgID)
        {
            try
            {
                ...
                string strRPTPathFile = AppDomain.CurrentDomain.BaseDirectory + UtilHelper.GetAppSettings("ReportTemplate") + "newwork_list.rpt";
