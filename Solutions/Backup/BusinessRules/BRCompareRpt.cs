...
namespace CSIPKeyInGUI.BusinessRules
{
    ...
    {    
        ...
        //增加【異動後自扣銀行帳號】欄位 add by xiongxiaofeng 130523 end
                    ...
                    "FROM Other_Bank_Temp A,(select distinct user_id,User_name from csip.dbo.M_USER) as  B " +
        ...
        private const string SEARCH_CHINA_TRUST = @"select B.USER_NAME,A.* " +
                    "FROM AUTO_PAY A,(select distinct user_id,User_name from csip.dbo.M_USER) as B " +
        ...
        private const string SEL_CARD_SURPLY_1 = @"select B.USER_NAME,A.Card_No,A.Eng_Name," +
                    ...
                    "FROM ReIssue_Card A,(select distinct user_id,User_name from csip.dbo.M_USER) as B " +
                    "where A.mod_date between @SearchStart and @SearchEnd " +
        ...
        private const string SEL_BLOCKCODE_ADD = @"select B.USER_NAME,A.* " +
                    "FROM Change_BLK A,(select distinct user_id,User_name from csip.dbo.M_USER) as B " +
                    "where A.mod_date between @SearchStart and @SearchEnd " +
        ...
        private const string SEL_SHOP_MODIFY = @"select D.USER_NAME,C.shop_id,C.Change_Item," +
                    ...
                    "from shop_Upload C,(select distinct user_id,User_name from csip.dbo.M_USER) as D " +
        ...
        private const string SEL_SHOP_ReqAppro = @"select b.mod_date,b.shop_id, b.before, b.after, b.record_name, b.business_name, b.merchant_name, d.user_name " +
                      "from (select * from shop_reqappro where keyin_flag='2' and newCreate_flag='N') as b, (select distinct user_id,User_name from csip.dbo.M_USER) as d " +
        ...
        private const string SEL_SHOP_BASIC = @"select B.USER_NAME,recv_no + '/'+ A.uni_no1+'-'+A.uni_no2 as MerNo,'','','','','','',''," +
                                                ...
                                                "FROM shop_basic A,(select distinct user_id,User_name from csip.dbo.M_USER) as  B " +
        ...
        private const string SEL_SHOP_PCAM = @"select B.USER_NAME,A.merchant_nbr,'','','','','','',''," +
                                                ...
                                                "FROM shop_pcam A,(select distinct user_id,User_name from csip.dbo.M_USER) as  B " +
                                                "where A.keyin_day between @SearchStart and @SearchEnd and " +
        ...
        private const string SEL_SHOP_ReqAppro_I = @"select b.mod_date,b.shop_id, b.after, b.record_name, b.business_name, b.merchant_name, d.user_name " +
                      "from (select * from shop_reqappro where keyin_flag='2' and newCreate_flag='Y') as b, (select distinct user_id,User_name from csip.dbo.M_USER) as d " +
                      "where b.mod_date between @SearchStart and @SearchEnd and b.user_id=d.user_id " +
                      "order by b.mod_date,substring(b.mod_time,1,4),b.shop_id";
        ...
        /*private const string SEARCH_AUTO_PAY_POPUL = @"select B.USER_NAME ,Convert(varchar(8),A.mod_date,112) AS mod_date, " +
                    ...
                    " FROM Auto_Pay_Popul A ,(select distinct user_id,User_name from csip.dbo.M_USER) as  B  " +
            ...
            try
            {
                ...
                switch (strProperty)    //* 業務項目
                {
                    ...
                    case "01010600":
                        //他行自扣(卡人資料異動)
                        strIs2Key = "Y";
                        strSqlSearch = SEARCH_OTHER_BANK_CARD;
                        break;
                    case "01010800":
                        //中信及郵局自扣(卡人資料異動)
                        strIs2Key = "Y";
                        strSqlSearch = SEARCH_CHINA_TRUST;
                        break;
                    case "01011000":
                        //訊息/更正單自扣(卡人資料異動)
                        strIs2Key = "Y";
                        strSqlSearch = SEARCH_UPDATE_INFO;
                        break;
                    //新增自扣終止（卡人資料異動） edit by xiongxiaofeng 130524 start
                    case "01011200":
                        //自扣終止與推廣員異動
                        strIs2Key = "Y";
                        strSqlSearch = SEARCH_AUTO_PAY_POPUL;
                        break;
                    //新增自扣終止（卡人資料異動） edit by xiongxiaofeng 130524 end
                    case "01020100":
                        //註銷(卡片資料異動)
                    ...
                    case "01020700":
                        //毀補(卡片資料異動)
                        strIs2Key = "Y";
                        strSqlSearch = SEL_CARD_SURPLY_1;
                        break;
                    case "01021300":
                        //掛補(卡片資料異動)
                        strIs2Key = "Y";
                        strSqlSearch = SEL_CARD_SURPLY_2;
                        break;
                    case "01020900":
                        // BlockCode新增/異動(卡片資料異動)
                        strIs2Key = "Y";
                        strSqlSearch = SEL_BLOCKCODE_ADD;
                        break;
                    case "01021100":
                        // BlockCode解除管制(卡片資料異動)
                        strIs2Key = "Y";
                        strSqlSearch = SEL_BLOCKCODE_NOT_CONTROL;
                        break;
                    case "01030200":
                        //特店資料異動(資料異動)
                        strIs2Key = "Y";
                        strSqlSearch = SEL_SHOP_MODIFY;
                        break;
                    case "01030100":
                        //特店資料異動(資料異動/請款加批)
                        strIs2Key = "Y";
                        strSqlSearch = SEL_SHOP_ReqAppro;
                        break;
                    case "01030201":
                        //特店資料異動(費率)
                        strIs2Key = "Y";
                        strSqlSearch = SEL_SHOP_RATE;
                        break;
                    case "01030202":
                        //特店資料異動(帳號)
                        strIs2Key = "Y";
                        strSqlSearch = SEL_SHOP_ACCOUNT;
                        break;
                    case "01030203":
                        //特店資料異動(解約)
                        strIs2Key = "Y";
                        strSqlSearch = SEL_SHOP_FIRE;
                        break;
                    case "01030204":
                        //特店資料異動(機器)
                        strIs2Key = "Y";
                        strSqlSearch = SEL_SHOP_MACHINE;
                        break;
                    case "01040101":
                        //特店資料新增(6001)
                        strIs2Key = "Y";
                        strSqlSearch = SEL_SHOP_BASIC;
                        break;
                    case "01040101M":
                        //特店資料新增(6001)會員附加服務
                        strIs2Key = "Y";
                        strSqlSearch = SEL_SHOP_BASIC_M;
                        break;
                    case "01040201":
                        //特店資料新增(PCAM)
                        strIs2Key = "Y";
                        strSqlSearch = SEL_SHOP_PCAM;
                        break;
                    case "01041100":
                        //特店資料新增(特店延伸性/請款加批)
                        strIs2Key = "Y";
                        strSqlSearch = SEL_SHOP_ReqAppro_I;
                        break;
                    case "01021500":
                        // 毀補轉一卡通
                        strIs2Key = "Y";
                ...
                if (strIs2Key != "Y")
                {
                    strSqlSearch = string.Format(SEL_COMMON_SQL, strAction);
                }
        ...
        /// 取符合條件的報表數據，并生成報表
        /// </summary>
        /// <param name="strProperty">業務項目ID</param>
        /// <param name="strPropertyName">業務項目名稱</param>
        /// <param name="strSearchStart">區間起</param>
        /// <param name="strSearchEnd">區間迄</param>
        /// <param name="rptResult">返回報表對象</param>
        /// <param name="strMsgID">傳回的消息ID號</param>
        /// <returns>返回執行標識：True---執行成功；False----執行失敗</returns>
        public static bool Report(string strProperty, string strPropertyName, string strSearchStart,
                                string strSearchEnd, out ReportDocument rptResult, 
                                ref string strMsgID)
        {
            try
            {
                //* 取報表數據
                Int64 lngTotal = 0;
                DataTable dtblSearchResult = SearchRPTData(strProperty, strSearchStart, strSearchEnd, ref lngTotal, ref strMsgID);
                ...
                string strRPTPathFile = AppDomain.CurrentDomain.BaseDirectory + UtilHelper.GetAppSettings("ReportTemplate") + "rptcompare.rpt";
                ...
                //* 業務項目
                rptResultTemp.DataDefinition.FormulaFields["RptType"].Text = "'業務項目：" + strPropertyName + "'";
                //* 總量
                rptResultTemp.DataDefinition.FormulaFields["iTotal"].Text = "'總量：" + lngTotal.ToString("N0") + "'";
